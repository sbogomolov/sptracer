...
namespace SPTracer
{
		  ...
		  holes_(std::move(holes))
	{
		...
		for (size_t j = 0; j < holes_.size(); j++)
		{
			...
			const Vec3& v1 = *hole[0];
			for (size_t i = 0; i < hole.size() - 2; i++)
			{
				const Vec3& v2 = *hole[i + 1];
				const Vec3& v3 = *hole[i + 2];
		...
		// compute outline normals
		outlineNormals_.reserve(outline_.size() - 2);

		const Vec3& v1 = *outline_[0];
		for (size_t i = 0; i < outline_.size() - 2; i++)
		{
			const Vec3& v2 = *outline_[i + 1];
			const Vec3& v3 = *outline_[i + 2];
	...
	bool PlanarMeshObject::Intersect(const Ray& ray, Intersection& intersection) const
	{
		...
		for (size_t j = 0; j < holes_.size(); j++)
		{
			...
			const Vec3& v1 = *hole[0];
			for (size_t i = 0; i < holeNormals.size(); i++)
			{
				const Vec3& v2 = *hole[i + 1];
				const Vec3& v3 = *hole[i + 2];
		...
		const Vec3& v1 = *outline_[0];
		for (size_t i = 0; i < outlineNormals_.size(); i++)
		{
			const Vec3& v2 = *outline_[i + 1];
			const Vec3& v3 = *outline_[i + 2];
